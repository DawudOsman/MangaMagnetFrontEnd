...
function SearchContainer(props)
{
    ...
    useEffect(()=>
      {
        var str = "?"
        for(let i = 0; i < fitlerDict.length; i++)
          {
            if(filterValues[i].length > 0)
              {
                ...
                if(i == 0)
                  {
                    subString = subString + filterObj[i].valuesMap[filterValues[i]]
                  }
                else
                {
                  for(let j = 0; j < filterValues[i].length; j++)
                    {
                      if(filterObj[i].valuesMap[filterValues[i]] !== undefined)
                        {
                          subString =  subString +  fitlerDict[i] + "[]=" + filterObj[i].valuesMap[filterValues[i]] + "&"
                        }
                        else
                        {
                      subString =  subString +  fitlerDict[i] + "[]=" + filterValues[i][j] + "&"
                        }
                    }
                }
                str = str + subString 
          ...
          props.changeSearchUrl(str)
          console.log(str)
          console.log(filterValues)
      }
  ,)
    return(<><div id={styles.SearchInputContainer}><input  id={styles.SearchInput} placeholder={props.searchUrl}></input><button onClick={()=>{toggleFitlers(!showFilters); changeFilterShown(-1)}} id={styles.FilterBtn}>{showFilters ? "Hide Filters" : "Show Filters"}<svg width={24} height={24} fill='none' stroke='Black' strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} viewBox='0 0 24 24' ><path d="m6 9 6 6 6-6"></path></svg></button></div>
